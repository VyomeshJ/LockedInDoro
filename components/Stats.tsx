...
export default function Stats({
    ...
    return (
      <div className="w-full h-full flex flex-col items-center justify-center px-6 gap-6 text-center overflow-hidden">
        ...
        <div className="max-w-xl w-full p-6 rounded-xl bg-[#182229] flex flex-col items-center gap-4">
          ...
          <span className="font-pixel text-lg opacity-80">
  ...
  return (
    <div className="w-full h-full min-h-0 flex flex-col items-center px-6 gap-6 overflow-hidden">
      <h1 className="font-pixel text-[4.5rem] leading-none shrink-0">
        Stats
      </h1>

      <div className="w-full max-w-xl flex flex-col gap-6 font-pixel text-2xl flex-1 min-h-0 overflow-hidden">
        <div className="grid grid-cols-2 gap-4 shrink-0">
          <div className="flex flex-col gap-2 p-4 rounded-xl bg-[#182229] min-w-0">
            <span className="text-lg opacity-80">Total Studied</span>
            <span className="text-3xl">{formatMinutes(totalMinutes)}</span>
          </div>

          <div className="flex flex-col gap-2 p-4 rounded-xl bg-[#182229] min-w-0">
            <span className="text-lg opacity-80">Today</span>
            <span className="text-3xl">{formatMinutes(todayMinutes)}</span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 p-4 rounded-xl bg-[#182229] flex-1 min-h-0 overflow-hidden">
          <div className="flex justify-between items-center shrink-0 gap-4">
            ...
            <span className="text-lg opacity-80 text-right">
              Minutes studied
            </span>
          </div>
          ...
          <div className="flex-1 min-h-[220px] flex items-end justify-between gap-3">
            {weekStats.map((day, index) => {
              ...
              return (
                <div
                  key={`${day.label}-${index}`}
                  className="flex-1 h-full flex flex-col justify-end items-center gap-3 min-w-0"
                >
                  <span className="text-sm opacity-80 h-5">
